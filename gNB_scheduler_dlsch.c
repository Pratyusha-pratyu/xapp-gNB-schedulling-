...
#include "common/ran_context.h"   // to get RC (this one is OK here)
...
//#define UE1_RATIO 0.40f   // SST=2 slice gets 40% max
//#define UE2_RATIO 0.60f   // SST=3 slice gets 60% max

// ------------------ Begin: OAI helper for xApp DL control ------------------
// This function uses only OAI types. FlexRIC will call it from its own code.
                                       ...
                                       int current_harq_pid)
{
  ...
  /* ---------- SLICE-AWARE HARQ MCS CAP ---------- */
  uint8_t ue_sst = sched_ctrl->xapp_sst;

  if (ue_sst == 2) {
    new_sched.mcs = min(new_sched.mcs, 6);
  } else if (ue_sst == 3) {
    new_sched.mcs = min(new_sched.mcs, 18);
  }

  new_sched.Qm = nr_get_Qm_dl(new_sched.mcs, dl_bwp->mcsTableIdx);
  new_sched.R  = nr_get_code_rate_dl(new_sched.mcs, dl_bwp->mcsTableIdx);

  LOG_D(NR_MAC,
        "[HARQ-MCS] UE %04x SST %u capped HARQ MCS=%d\n",
        UE->rnti, ue_sst, new_sched.mcs);
  /* -------------------------------------------- */
...
static uint32_t pf_tbs[3][29] __attribute__((unused));  // pre-computed, approximate TBS values for PF coefficient
typedef struct UEsched_s {
  float coef;
  NR_UE_info_t * UE;
  int sst;          // slice identifier (e.g., 2, 3, or other)
  int prbs_alloc;   // PRBs actually allocated
  int tbs_bytes;    // TBS actually allocated (bytes)
  int selected_mcs;
} UEsched_t;
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  /* local extended UE schedule entry so we don't change global typedef */
  typedef struct {
    float coef;
    NR_UE_info_t *UE;
    int selected_mcs;
    int sst;        // slice identifier (e.g., 2, 3, or -1)
    int prbs_alloc; // PRBs actually allocated
    int tbs_bytes;  // TBS actually allocated (bytes)
  } UEsched_ext_t;

  /* Keep a copy of initial total PRBs per beam so we can apply % quotas */
  int total_prbs[num_beams];
  for (int i = 0; i < num_beams; i++) total_prbs[i] = n_rb_sched[i];
  
  /* Track per-slot allocated PRBs per SST and beam */
  int allocated_per_sst[256][num_beams];
  memset(allocated_per_sst, 0, sizeof(allocated_per_sst));

  /* UEs that could be scheduled (extended) */
  UEsched_ext_t UE_sched[MAX_MOBILES_PER_GNB + 1];
  memset(UE_sched, 0, sizeof(UE_sched));
  ...
  /* Build UE list (unchanged retransmission logic from your original) */
  UE_iterator(UE_list, UE) {
    ...
    if (harq_pid >= 0) {
      ...
      /* Calculate coeff and select MCS (matching your original) */
      ...
      uint32_t tbs = nr_compute_tbs(Qm,
                                    ...
                                    0 /* tb_scaling */,
                                    l) >> 3;
      float coeff_ue = (float) tbs / (UE->dl_thr_ue > 0.0f ? UE->dl_thr_ue : 1.0f);
      ...
      /* Create UE_sched list entry for new transmission attempts */
      UE_sched[numUE].coef = coeff_ue;
      UE_sched[numUE].UE = UE;
      UE_sched[numUE].selected_mcs = selected_mcs;
      UE_sched[numUE].sst = sched_ctrl->xapp_sst;
      UE_sched[numUE].prbs_alloc = 0;
      UE_sched[numUE].tbs_bytes = 0;
      numUE++;
    }
  } // end UE_iterator

    /* use existing file-scope comparator */
  qsort(UE_sched, numUE, sizeof(UE_sched[0]), comparator);


  const int min_rbSize = 5;

  for (int i = 0; i < numUE; i++) {
    UEsched_ext_t *entry = &UE_sched[i];
    if (entry->UE == NULL) break;

    NR_UE_sched_ctrl_t *sched_ctrl = &entry->UE->UE_sched_ctrl;
    const uint16_t rnti = entry->UE->rnti;

    NR_UE_DL_BWP_t *dl_bwp = &entry->UE->current_DL_BWP;
    NR_UE_UL_BWP_t *ul_bwp = &entry->UE->current_UL_BWP;
    ...
    NR_beam_alloc_t beam = beam_allocation_procedure(&mac->beam_info, frame, slot, entry->UE->UE_beam_index, slots_per_frame);
    ...
    if (remainUEs[beam.idx] == 0 || n_rb_sched[beam.idx] < min_rbSize) {
      reset_beam_status(&mac->beam_info, frame, slot, entry->UE->UE_beam_index, slots_per_frame, beam.new_beam);
      continue;
    }

    /* MCS has been set earlier in building list */
    ...
    bwp_info_t bwp_info = get_pdsch_bwp_start_size(mac, entry->UE);
    ...
    /* Find PUCCH occasion as before */
    int alloc = -1;
    if (!get_FeedbackDisabled(entry->UE->sc_info.downlinkHARQ_FeedbackDisabled_r17, sched_ctrl->available_dl_harq.head)) {
      int r_pucch = nr_get_pucch_resource(sched_ctrl->coreset, ul_bwp->pucch_Config, CCEIndex);
      alloc = nr_acknack_scheduling(mac, entry->UE, frame, slot, entry->UE->UE_beam_index, r_pucch, 0);
      if (alloc < 0) {
    ...
    NR_sched_pdsch_t sched_pdsch = {
      .rbStart = rbStart,
      .mcs = entry->selected_mcs,
      .R = nr_get_code_rate_dl(entry->selected_mcs, dl_bwp->mcsTableIdx),
      .Qm = nr_get_Qm_dl(entry->selected_mcs, dl_bwp->mcsTableIdx),
      .dl_harq_pid = sched_ctrl->available_dl_harq.head,
      .pucch_allocation = alloc,
      .pm_index = get_pm_index(mac, entry->UE, dl_bwp->dci_format, l, mac->radio_config.pdsch_AntennaPorts.XP),
      .nrOfLayers = l,
      .bwp_info = bwp_info,
    ...
    /* Determine SST assigned by xApp */
    int ue_sst = sched_ctrl->xapp_sst;


    /* Strict quotas per beam — xApp controlled */
    int quota_remaining = n_rb_sched[beam.idx];

    float ratio = -1.0f;

    if (ue_sst >= 0)
      ratio = mac->xapp_sst_prb_share[ue_sst];

    if (ratio > 0.0f) {

      int cap = (int)floor(total_prbs[beam.idx] * ratio + 0.0001f);


      quota_remaining = cap - allocated_per_sst[ue_sst][beam.idx];

      if (quota_remaining <= 0) {
        LOG_D(NR_MAC,
              "[UE %04x][%4d.%2d] SST=%d quota exhausted on beam %d, skipping UE\n",
              rnti, frame, slot, ue_sst, beam.idx);

        reset_beam_status(&mac->beam_info,
                          frame, slot,
                          entry->UE->UE_beam_index,
                          slots_per_frame,
                          beam.new_beam);
        continue;
      }
    ...
    if (quota_remaining < min_rbSize) {
      LOG_D(NR_MAC,
            "[UE %04x][%4d.%2d] quota remaining %d < min_rbSize, skipping\n",
            rnti, frame, slot, quota_remaining);
    ...
    /* Cap effective maximum by quota */
    uint16_t effective_max_rbSize = max_rbSize;
    if (quota_remaining < effective_max_rbSize)
      effective_max_rbSize = (uint16_t)quota_remaining;

    /* Proportional share inside SST quota */
    if (ratio > 0.0f) {

      double denom = 0.0;

      for (int j = 0; j < numUE; j++) {
        if (!UE_sched[j].UE) continue;

        if (UE_sched[j].sst == ue_sst)
          denom += UE_sched[j].coef;
      }

      if (denom > 0.0) {

        int allowed_by_share =
          (int)floor(((double)quota_remaining) *
                     ((double)entry->coef) / denom + 0.000001);

        if (allowed_by_share < min_rbSize) {
          if (quota_remaining >= min_rbSize)
            allowed_by_share = min_rbSize;
          else
            allowed_by_share = 0;
        }

        if (allowed_by_share > 0 &&
            allowed_by_share < effective_max_rbSize)
          effective_max_rbSize = (uint16_t)allowed_by_share;
      }
    }


    /* Find rbSize and TBS subject to effective_max_rbSize (quota-aware) */
    uint32_t TBS = 0;
    uint16_t rbSize = 0;
    nr_find_nb_rb(sched_pdsch.Qm,
                  ...
                  min_rbSize,
                  effective_max_rbSize,
                  &TBS,
                  &rbSize);

    if (rbSize < min_rbSize) {
      LOG_D(NR_MAC, "[UE %04x][%4d.%2d] rbSize %d < min_rbSize or cannot fit, skipping\n", rnti, frame, slot, rbSize);
      reset_beam_status(&mac->beam_info, frame, slot, entry->UE->UE_beam_index, slots_per_frame, beam.new_beam);
      continue;
    }

    /* Ensure rbSize does not exceed available n_rb_sched for beam */
    if (rbSize > n_rb_sched[beam.idx]) {
      if (n_rb_sched[beam.idx] < min_rbSize) {
        reset_beam_status(&mac->beam_info, frame, slot, entry->UE->UE_beam_index, slots_per_frame, beam.new_beam);
        continue;
      }
      uint16_t adjust_max = (uint16_t)n_rb_sched[beam.idx];
      nr_find_nb_rb(sched_pdsch.Qm,
                    sched_pdsch.R,
                    1,
                    sched_pdsch.nrOfLayers,
    ...
    /* Track allocation in UE_sched entry */
    entry->prbs_alloc = sched_pdsch.rbSize;
    entry->tbs_bytes  = sched_pdsch.tb_size;
    entry->sst = ue_sst;

    /* Reduce scheduling bookkeeping */
    n_rb_sched[beam.idx] -= sched_pdsch.rbSize;
    
    /* Update per-slice allocation tracking (dynamic) */
    if (ue_sst >= 0)
      allocated_per_sst[ue_sst][beam.idx] += sched_pdsch.rbSize;


    /* Mark rballoc_mask bits */
    for (int rb = 0; rb < sched_pdsch.rbSize; rb++)
      rballoc_mask[bwp_start + sched_pdsch.rbStart + rb] |= slbitmap;

    /* Finally call post process (same as original) */
    post_process_dlsch(mac, pp_pdsch, entry->UE, &sched_pdsch);

    remainUEs[beam.idx]--;
  } // end for each sorted UE

  int total_alloc_prbs = 0;

  for (int b = 0; b < num_beams; b++)
    total_alloc_prbs += (total_prbs[b] - n_rb_sched[b]);

  if (total_alloc_prbs > 0) {

    LOG_I(NR_MAC,
          "Frame %d Slot %d | Total allocated PRBs: %d\n",
          frame, slot, total_alloc_prbs);

    /* Per-slice summary */
    for (int s = 0; s < 256; s++) {

      if (mac->xapp_sst_prb_share[s] > 0.0f) {

        int slice_prbs = 0;

        for (int b = 0; b < num_beams; b++)
          slice_prbs += allocated_per_sst[s][b];

        if (slice_prbs > 0) {

          LOG_I(NR_MAC,
                "   SST %d → %d PRBs (ratio %.2f)\n",
                s,
                slice_prbs,
                mac->xapp_sst_prb_share[s]);
        }
      }
    }

    /* Per-UE logging (dynamic SST) */
    for (int i = 0; i < numUE; i++) {

      if (!UE_sched[i].UE || UE_sched[i].prbs_alloc == 0)
        continue;

      double tbs_bits = (double)UE_sched[i].tbs_bytes * 8.0;
      double slot_duration_sec =
          1.0 / (mac->frame_structure.numb_slots_frame * 1000.0);
      double throughput_mbps =
          (tbs_bits / slot_duration_sec) / 1e6;

      LOG_I(NR_MAC,
            "   UE RNTI=0x%04x | SST=%d | PRBs=%d | TBS=%d bytes (~%.2f Mbps)\n",
            UE_sched[i].UE->rnti,
            UE_sched[i].sst,
            UE_sched[i].prbs_alloc,
            UE_sched[i].tbs_bytes,
            throughput_mbps);
    }
    ...
    LOG_D(NR_MAC,
          "No data allocated this slot (frame %d slot %d)\n",
          frame, slot);
  }
