...
def slice_ind_to_dict_json(ind):
    ...
    if ind.slice_stats.dl.len_slices <= 0:
        ...
        if ind.slice_stats.dl.sched_name:
            dl_dict["ue_sched_algo"] = ind.slice_stats.dl.sched_name[0]
        else:
            dl_dict["ue_sched_algo"] = "null"
        ...
        for s in ind.slice_stats.dl.slices:
            ...
            label = s.label[0] if s.label else "null"
            sched = s.sched[0] if s.sched else "null"
    ...
    "slices": [
        {
            ...
            "slice_algo_params": {"pct_rsvd": 0.4},
        },
        {
            "id": 3,
            "label": "s2",
            "ue_sched_algo": "PF",
            "type": "SLICE_SM_NVS_V0_CAPACITY",
            "slice_algo_params": {"pct_rsvd": 0.6},
        }
    ]
}
...
assoc_ue_slice = {
    "num_ues": 2,
    "ues": [
        {"assoc_dl_slice_id": 2},  # First UE → SST 2
        {"assoc_dl_slice_id": 3},  # Second UE → SST 3
    ]
}
...
def fill_slice_ctrl_msg(ctrl_type, ctrl_msg):
    ...
    if (ctrl_type == "ADDMOD"):
        ...
        # Protect against mismatch between requested UEs and available RNTIs
        msg.u.ue_slice.len_ue_slice = min(ctrl_msg["num_ues"], len(rnti_list))

        assoc = ric.ue_slice_assoc_array(msg.u.ue_slice.len_ue_slice)

        for i in range(msg.u.ue_slice.len_ue_slice):

            a = ric.ue_slice_assoc_t()
            a.rnti = rnti_list[i]   # Keep this
...
msg = fill_slice_ctrl_msg("ADDMOD", add_nvs_slices_cap)
...
# Wait until we detect 2 UEs
while len(rnti_list) < 2:
    print("Waiting for at least 2 UEs...")
    time.sleep(1)

print("Active RNTIs detected:", rnti_list)

assoc_ue_slice = {
    "num_ues": 2,
    "ues": [
        {"assoc_dl_slice_id": 2},  # First detected UE → SST 2
        {"assoc_dl_slice_id": 3},  # Second detected UE → SST 3
    ]
}
